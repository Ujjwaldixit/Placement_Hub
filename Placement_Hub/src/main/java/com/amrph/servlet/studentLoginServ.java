...
public class studentLoginServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet studentLoginServ</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet studentLoginServ at " + request.getContextPath() + "</h1>");
            String username=request.getParameter("username");
            String password=request.getParameter("password");
            StudentDao StudentDao=new StudentDao(connection_provider.getconnection());
            student student=StudentDao.getNameAndPassword(username, password);
            if(student==null)
            {
