...
public class deleteDrive extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet deleteDrive</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet deleteDrive at " + request.getContextPath() + "</h1>");
            String company_name=request.getParameter("job_no");
            out.println(company_name);           
            drivesDao drivesDao=new drivesDao(connection_provider.getconnection());
            drivesDao.deleteDrive(company_name);
            AdminDao admindao=new AdminDao(connection_provider.getconnection());
            admindao.deleteCompanyDrive(company_name);
          //  out.println(job_no);
